...
class Locations {
    ...
    // Лист для автокомплита

    createShortCitiesList(cities) {
        //{ 'City, Country': null } надо
        //entries возвращает [key, value]
    ...
    async fetchTickets(params) {
        ...
        let newArray = [];
        this.lastSearch.forEach(function (elem) {
            newArray[elem.id] = elem;
        });
        this.idSearch = newArray;
    }
    ...
    serializeTickets(tickets) {
        return Object.values(tickets).map((ticket) => {
            return {
                ...ticket,
                id: `${ticket.airline}${ticket.flight_number}${this.formatDate(
                    ticket.departure_at,
                    "ddMMyyyyhhmm"
                )}`,
